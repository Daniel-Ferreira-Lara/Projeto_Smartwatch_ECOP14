...
unsigned char nome[16],matricula[16];
unsigned char change=1,crono,tempos,som=0;
unsigned char musica=1,tela=0,freq=2;
unsigned int temperatura=01;
static const int radio[] = {883,901,963,1005,1016,1074,1108,1185};
static const char semana[] = {'D','o','m','S','e','g','T','e','r','Q','u','a','Q','u','i','S','e','x','S','a','b'};
...
void IMC(){
    ...
    lcdPosition(1,0);
    if(imc<15||imc>29)
        rgbColor(1),lcdString("Procure o medico");
    else if(imc<18||imc>25)
        rgbColor(3),lcdString("Se alimente bem");
    else
        rgbColor(2),lcdString("Ok, Saude otima");
    atraso_ms(3000);
}
...
void data(){
    ...
    for(i=(bcd2dec(dsReadData(3)& 0x07))*3;i<(bcd2dec(dsReadData(3)& 0x07))*3+3;i++)
        lcdChar(semana[i]);
    lcdChar(' ');
                ...
                for(i=0;i<5;i++){
                    ...
                    if (kpRead() != tecla){
                        ...
                        tecla = kpRead();
                            ...
                            if (((tecla) & (1<<(0)))&&freq<7) freq++,tela=2;
                            if (((tecla) & (1<<(2)))&&freq) freq--,tela=2;
                            if (((tecla) & (1<<(1)))&&musica) musica--,tela=1;
                            if (((tecla) & (1<<(3)))&&musica<5) musica++,tela=1;

                            if (((tecla) & (1<<(4))))tela=0;

                            if (((tecla) & (1<<(7)))){if(tela==3)tempos=0,crono=1;tela=3;}
                            if (((tecla) & (1<<(6)))) tela=3,crono=!crono;
                    }
                }
                processos=2;
                break;
